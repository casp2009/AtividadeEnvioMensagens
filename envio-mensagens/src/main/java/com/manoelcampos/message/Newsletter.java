...
public class Newsletter {
     ...
     */
    public void sendMessage(MessageService messageService, String destination, String msg) {
        messageService.send(destination, msg);
    }

    public void send(final String msgTemplate) {
        for (final Customer customer : customers) {
            sendMessage(new WhatsApp(),customer.getPhone(), formatMsg(customer, msgTemplate));
            sendMessage(new Email(),customer.getEmail(), formatMsg(customer, msgTemplate));
            sendMessage(new Sms(),customer.getPhone(), formatMsg(customer, msgTemplate));
        }
    }
